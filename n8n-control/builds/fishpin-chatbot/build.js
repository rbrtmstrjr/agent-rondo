// Assembles fishpin-chatbot.workflow.json — the FishPin deployment of the
// support-chatbot (RAG) engine. Run: node build.js
const fs = require('fs');
...
const WEBHOOK_PATH = 'chat-fishpin';
...
const nodes = [
  {
    ...
    position: pos(-200, 300), webhookId: 'chat-fishpin-hook',
  },
  {
    parameters: {
      assignments: { assignments: [
        { id: 'c1', name: 'companyName', value: 'FishPin', type: 'string' },
        { id: 'c2', name: 'supportContact', value: 'fishpinsupport@gmail.com', type: 'string' },
  ...
  slackNode('node-log', 'Log to Slack',
    ...
    '={{ "🐟 *FishPin Chatbot* | session `" + $(\'Decide\').first().json.sessionId + "` | score " + $(\'Decide\').first().json.score + ($(\'Decide\').first().json.escalate ? " | ⚠️ ESCALATED" : "") + "\\n*Q:* " + $(\'Decide\').first().json.message + "\\n*A:* " + $(\'Decide\').first().json.reply }}',
    1340, 300),
  {
    parameters: {
      conditions: {
  ...
  slackNode('node-escalate', 'Notify Support (Human)',
    ...
    '={{ "🚨 *FishPin — human handoff needed* — the bot could not answer this from the knowledge base.\\n*Session:* `" + $(\'Decide\').first().json.sessionId + "`\\n*User asked:* " + $(\'Decide\').first().json.message + "\\n*Top match score:* " + $(\'Decide\').first().json.score + "\\nPlease follow up." }}',
    1780, 180),
  {
    parameters: {
      respondWith: 'json',
...
const workflow = {
  name: 'FishPin Support Chatbot (RAG) — Live',
  nodes, connections,
  settings: { executionOrder: 'v1', errorWorkflow: '660Xkpo164VSNTDZ' },
};

const out = path.join(__dirname, 'fishpin-chatbot.workflow.json');
