...
section('brand', 'Brand bible', () => {
  ...
  check('system prompt lists every banned word',
    B.BANNED_WORDS.every(w => sys.toLowerCase().includes(w.toLowerCase())));
  ...
  check('user prompt carries the pillar rule', u1.includes('feature spotlight'));
...
- Produces:
  ...
  - `readImageSize(buf) -> { width, height, type } | null` (PNG and JPEG)
